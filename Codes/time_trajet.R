...
# UI
ui <- fluidPage(
  titlePanel("Total Travel Time by Type of Transport and Part of Day for Luxembourg"),
  fluidRow(
    column(width = 3,
...
server <- function(input, output) {
  output$distancePlot <- renderPlot({
    if (length(input$typeSelect) == 0) {
      ...
      return(ggplot() + 
               labs(x = "Part of Day", y = "Total Travel Time (hours)", fill = "Type of Transport") +
               ggtitle("Please select at least one type of transport and type of day to view data."))
    } else {
      ...
      if(input$filterSelect == "Per hour") {
        ...
        plot_0_to_12 <- ggplot(filtered_data_0_to_12, aes(x = part_of_day, fill = subtrip_type)) +
          geom_histogram(stat = "count", position = position_dodge(), binwidth = 1) +
